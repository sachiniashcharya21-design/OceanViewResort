...
<%
    ...
    String successMessage = (String) session.getAttribute("customersSuccessMessage");
    String errorMessage = (String) session.getAttribute("customersErrorMessage");
    if (successMessage != null) session.removeAttribute("customersSuccessMessage");
    if (errorMessage != null) session.removeAttribute("customersErrorMessage");
    ...
    try {
        ...
        if ("addCustomer".equals(action) && "POST".equalsIgnoreCase(request.getMethod())) {
            ...
            String nicPassportNormalized = (nicPassport != null && !nicPassport.trim().isEmpty()) ? nicPassport.trim() : null;
            ...
            checkPs.setString(1, nicPassportNormalized);
            ...
            if (checkRs.next()) {
                ...
                ps.setString(2, nicPassportNormalized);
        ...
        if ("editCustomer".equals(action) && "POST".equalsIgnoreCase(request.getMethod())) {
            ...
            String nicPassportNormalized = (nicPassport != null && !nicPassport.trim().isEmpty()) ? nicPassport.trim() : null;
            PreparedStatement checkPs = conn.prepareStatement("SELECT guest_id FROM guests WHERE nic_passport = ? AND guest_id != ?");
            checkPs.setString(1, nicPassportNormalized);
        ...
        boolean redirectAfterAction =
            ("addCustomer".equals(action) && "POST".equalsIgnoreCase(request.getMethod())) ||
            ("editCustomer".equals(action) && "POST".equalsIgnoreCase(request.getMethod())) ||
            "deleteCustomer".equals(action);
        if (redirectAfterAction) {
            if (successMessage != null) session.setAttribute("customersSuccessMessage", successMessage);
            if (errorMessage != null) session.setAttribute("customersErrorMessage", errorMessage);
            if (conn != null) { try { conn.close(); } catch (Exception ignore) {} }
            response.sendRedirect(request.getContextPath() + "/admin/admin-customers.jsp");
            return;
        }
...
<head>
    ...
    <title>Customer Management - Ocean View Resort Admin</title>
...
<body>
    ...
    <main class="main-content">
        ...
        <div class="register-card">
            ...
            <form method="POST" action="<%= request.getContextPath() %>/admin/admin-customers.jsp" id="registerForm">
                        ...
                        <tr>
                            <td>
                                <div class="customer-info">
                                    ...
                                    <div class="customer-details">
                                        <h4><%= fullName %></h4>
                                        <p><%= email != null && !email.isEmpty() ? email : "-" %></p>
                                    </div>
                                </div>
                            </td>
                            <td><%= nicPassport != null ? nicPassport : "-" %></td>
                            <td><i class="fas fa-phone" style="color: var(--primary); margin-right: 5px;"></i><%= phone != null && !phone.isEmpty() ? phone : "-" %></td>
                            <td><span class="badge badge-nationality"><%= nationality != null && !nationality.isEmpty() ? nationality : "-" %></span></td>
                            ...
                            <td>
                                <button class="action-btn view" title="View Details" 
                                    data-id="<%= guestId %>" 
                                    data-name="<%= escapeHtmlAttr(fullName) %>" 
                                    data-nic="<%= escapeHtmlAttr(nicPassport != null ? nicPassport : "") %>" 
                                    data-email="<%= escapeHtmlAttr(email != null ? email : "") %>" 
                                    data-phone="<%= escapeHtmlAttr(phone != null ? phone : "") %>" 
                                    data-nationality="<%= escapeHtmlAttr(nationality != null ? nationality : "") %>" 
                                    data-address="<%= escapeHtmlAttr(address != null ? address : "") %>" 
                                    data-date="<%= escapeHtmlAttr(createdAt != null ? sdf.format(createdAt) : "") %>"
                                ...
                                <button class="action-btn delete" title="Delete Customer" data-id="<%= guestId %>" data-name="<%= escapeHtmlAttr(fullName) %>" onclick="deleteCustomerData(this)"><i class="fas fa-trash"></i> Delete</button>
                                <a href="<%= request.getContextPath() %>/admin/admin-book-room.jsp?guestId=<%= guestId %>&guestName=<%= java.net.URLEncoder.encode(fullName, "UTF-8") %>" class="action-btn book" title="Book Room"><i class="fas fa-calendar-plus"></i> Book</a>
                            </td>
                        </tr>
                        <%
                                }
    ...
    <div class="modal-overlay" id="editModal">
        <div class="modal">
            ...
            <form method="POST" action="<%= request.getContextPath() %>/admin/admin-customers.jsp">
        ...
        function deleteCustomer(id, name) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    window.location.href = '<%= request.getContextPath() %>/admin/admin-customers.jsp?action=deleteCustomer&guestId=' + id;
                }
            });
        }
