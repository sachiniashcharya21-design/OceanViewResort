...
<%
    ...
    String successMessage = (String) session.getAttribute("paymentsSuccessMessage");
    String errorMessage = (String) session.getAttribute("paymentsErrorMessage");
    if (successMessage != null) session.removeAttribute("paymentsSuccessMessage");
    if (errorMessage != null) session.removeAttribute("paymentsErrorMessage");
    ...
    try {
        ...
        if ("updatePayment".equals(action) && "POST".equalsIgnoreCase(request.getMethod())) {
            ...
            PreparedStatement ps = conn.prepareStatement(
                "UPDATE bills SET payment_status = ?, payment_method = ?, paid_at = CASE WHEN ? = 'PAID' THEN NOW() ELSE NULL END WHERE bill_id = ?"
            );
            ...
            if (successMessage != null) session.setAttribute("paymentsSuccessMessage", successMessage);
            if (errorMessage != null) session.setAttribute("paymentsErrorMessage", errorMessage);
            if (conn != null) { try { conn.close(); } catch (Exception ignore) {} }
            response.sendRedirect(request.getContextPath() + "/admin/admin-payments.jsp");
            return;
        }
...
<head>
    ...
    <title>Payment Management - Ocean View Resort Admin</title>
...
<body>
                ...
                <table class="data-table" id="pendingTable">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <a href="<%= request.getContextPath() %>/admin/admin-payment.jsp?reservationId=<%= pReservationId %>" 
                    ...
                    <tbody>
                        ...
                        <tr data-id="<%= billId %>" 
                            ...
                            <td>
                                ...
                                <a class="action-btn" href="<%= request.getContextPath() %>/admin/admin-payment.jsp?reservationId=<%= reservationId %>" style="background:var(--primary);color:white;">
                                    <i class="fas fa-credit-card"></i> Pay
                                </a>
                                <% } %>
                                <button class="action-btn edit" onclick="editPayment(<%= billId %>, '<%= payStatus %>', '<%= payMethod != null ? payMethod : "" %>')">
                                    <i class="fas fa-edit"></i> Edit
                                </button>
                                <a class="action-btn print" href="<%= request.getContextPath() %>/admin/print-invoice.jsp?id=<%= billId %>&type=bill" target="_blank">
    ...
    <div class="modal-overlay" id="editModal">
        <div class="modal">
            <div class="modal-header">
                <h3><i class="fas fa-edit"></i> Update Payment</h3>
                <button class="modal-close" onclick="closeModal('editModal')">&times;</button>
            </div>
            <form id="editPaymentForm" method="POST" action="<%= request.getContextPath() %>/admin/admin-payments.jsp">
                <div class="modal-body">
                    <input type="hidden" name="action" value="updatePayment">
                    <input type="hidden" name="billId" id="editBillId">
                    <div class="form-group">
                        <label><i class="fas fa-clipboard-check"></i> Payment Status</label>
                        <select name="paymentStatus" id="editStatus" class="form-control" required>
                            <option value="PENDING">Pending</option>
                            <option value="PARTIAL">Partial</option>
                            <option value="PAID">Paid</option>
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <select name="paymentMethod" id="editMethod" class="form-control">
    ...
    <script>
        ...
        Swal.fire({ icon: 'success', title: 'Success', text: '<%= escJs(successMessage) %>', confirmButtonColor: '#008080' });
        <% } %>
        <% if (errorMessage != null) { %>
        Swal.fire({ icon: 'error', title: 'Error', text: '<%= escJs(errorMessage) %>', confirmButtonColor: '#008080' });
        <% } %>
        ...
        function viewPaymentFromRow(button) {
            try {
                ...
                var html = '<div class="invoice-preview">' +
                    ...
                    '<img src="../images/logo.png" alt="Ocean View Resort" style="height: 50px; margin-bottom: 8px;">' +
                ...
                document.getElementById('printFromModal').href = '<%= request.getContextPath() %>/admin/print-invoice.jsp?id=' + currentBillId + '&type=bill';
        ...
        // Edit payment
        function editPayment(billId, status, method) {
