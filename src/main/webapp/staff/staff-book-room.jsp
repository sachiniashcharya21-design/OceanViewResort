...
<%@ page import="java.sql.*, java.util.*, java.text.SimpleDateFormat, java.text.DecimalFormat, java.net.URLDecoder, java.net.URLEncoder" %>
<%! 
...
<%
    ...
    try { preselectedRoomId = Integer.parseInt(request.getParameter("roomId")); } catch (Exception ignore) {}
...
// Resolve guest name from DB if not present in URL
        ...
        if ("createBooking".equals(action)) {
            ...
            if (checkInDate == null || checkOutDate == null || checkInDate.isEmpty() || checkOutDate.isEmpty() || checkOutDate.compareTo(checkInDate) <= 0) {
...
<body>
    ...
    <div class="header">
        ...
        <div class="header-actions">
            <a href="<%= request.getContextPath() %>/staff/staff-customers.jsp" class="btn btn-back"><i class="fas fa-arrow-left"></i> Back to Customers</a>
    ...
    <div class="main-content">
        ...
        <div class="filter-section">
            ...
            <form method="GET" action="<%= request.getContextPath() %>/staff/staff-book-room.jsp" class="filter-form">
                <input type="hidden" name="guestId" value="<%= guestId %>">
                <input type="hidden" name="guestName" value="<%= URLEncoder.encode(guestName, "UTF-8") %>">
                ...
                <div class="filter-buttons">
                    ...
                    <a href="<%= request.getContextPath() %>/staff/staff-book-room.jsp?guestId=<%= guestId %>&guestName=<%= URLEncoder.encode(guestName, "UTF-8") %>" class="btn btn-clear"><i class="fas fa-times"></i> Clear</a>
                </div>
            </form>
        </div>
        ...
        <div class="rooms-grid">
            <%
                try {
                    // Build query with filters - aligned with rooms schema (floor_number, notes)
                    StringBuilder query = new StringBuilder();
                    query.append("SELECT r.room_id, r.room_number, r.room_type_id, r.floor_number AS floor, r.status, r.notes, ");
                    query.append("rt.type_name, rt.rate_per_night, rt.max_occupancy, rt.description AS type_description, rt.amenities AS type_amenities ");
                    ...
                    if (!filterRoomType.isEmpty()) {
                        try {
                            query.append("AND r.room_type_id = ").append(Integer.parseInt(filterRoomType)).append(" ");
    ...
    <div id="bookingModal" class="modal-overlay">
        <div class="modal-content">
            ...
            <form method="POST" action="<%= request.getContextPath() %>/staff/staff-book-room.jsp?guestId=<%= guestId %>&guestName=<%= URLEncoder.encode(guestName, "UTF-8") %>">
    ...
    <script>
        ...
        <% if (successMessage != null && newReservationId > 0) { %>
            Swal.fire({
                ...
                window.location.href = '<%= request.getContextPath() %>/staff/staff-payment.jsp?reservationId=<%= newReservationId %>';
            });
        <% } %>
        ...
        // If we arrived with a preselected room, open the booking modal for it (when visible).
        (function () {
            const preselectedRoomId = <%= preselectedRoomId %>;
            if (!preselectedRoomId) return;

            const btn = document.querySelector('.btn-book[data-room-id=\"' + preselectedRoomId + '\"]');
            if (btn) {
                selectRoom(btn);
            }
        })();
