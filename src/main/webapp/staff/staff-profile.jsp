...
<%
    ...
    try {
        ...
        // AJAX: Upload profile picture (returns JSON)
        if ("uploadProfilePic".equals(action) && "POST".equalsIgnoreCase(request.getMethod())) {
            ...
            if (imageData == null || imageData.trim().isEmpty()) {
                resultJson = "{\"success\": false, \"message\": \"No image data provided\"}";
            } else {
                try {
                    ...
                    if (decoded.length > (5 * 1024 * 1024)) {
                        resultJson = "{\"success\": false, \"message\": \"Image too large (max 5MB)\"}";
                    } else {
                        ...
                        resultJson = "{\"success\": true, \"imagePath\": \"" + jsonEscape(imagePath) + "\"}";
                    }
                } catch (Exception ex) {
                    resultJson = "{\"success\": false, \"message\": \"" + jsonEscape(ex.getMessage()) + "\"}";
                }
            }
        ...
        if ("changePassword".equals(action) && "POST".equalsIgnoreCase(request.getMethod())) {
            ...
            if (verifyRs.next()) {
                ...
                if (!currentOk) {
                    ...
                    PreparedStatement updatePs = conn.prepareStatement("UPDATE users SET password = ?, password_hash = ? WHERE user_id = ?");
                ...
                <div class="avatar-container">
                    <div class="avatar" onclick="document.getElementById('profilePicInput').click()">
                        <% if (profilePic != null && !profilePic.isEmpty()) { %>
                            <img id="profileImage" src="<%= request.getContextPath() %>/uploads/profiles/<%= profilePic %>" alt="Profile">
                        <% } else { %>
